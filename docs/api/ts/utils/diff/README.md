[**@pt9912/v-map**](../../README.md)

***

[@pt9912/v-map](../../README.md) / utils/diff

# utils/diff

## Interfaces

- [BuilderConfig](interfaces/BuilderConfig.md)
- [LayerGroup](interfaces/LayerGroup.md)
- [MapConfig](interfaces/MapConfig.md)
- [NormalizedLayer](interfaces/NormalizedLayer.md)
- [NormalizedStyle](interfaces/NormalizedStyle.md)

## Type Aliases

- [LayerPatch](type-aliases/LayerPatch.md)
- [LayersDiff](type-aliases/LayersDiff.md)
- [LayerType](type-aliases/LayerType.md)

## Functions

- [diffLayers](functions/diffLayers.md)
- [diffRelevantFields](functions/diffRelevantFields.md)
